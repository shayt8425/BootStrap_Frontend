...
//queueID给“进度条”加背景css的ID样式。文件选择后的容器ID

//queueSizeLimit允许多文件上传的数量。默认：999

//removeCompleted上传完成后队列是否自动消失。默认：true

//removeTimeout上传完成后队列多长时间后消失。默认 3秒需要：'removeCompleted' : true,时使用

//requeueErrors队列上传出错，是否继续回滚队列，即反复尝试上传。默认：false

//successTimeout上传超时时间。文件上传完成后,等待服务器返回信息的时间(秒).超过时间没有返回的话,插件认为返回了成功。 默认：30秒

//swfswf文件的路径,本文件是插件自带的,不可用其它的代替.本参数不可省略

//uploader上传处理程序URL，本参数不可省略
...
//onCancel当取消一个上传队列中的文件时触发,删除时触发 

//onClearQueue清除队列。当'cancel'方法带着*参数时,也就是说一次全部取消的时候触发.queueItemCount是被取消的文件个数（另外的按钮）
...
//settings返回或修改一个 uploadify实例的settings值

//stop停止当前的上传并重新添加到队列中去

//upload上传指定的文件或者所有队列中的文件
		...
		init : function(options, swfUploadOptions) {
			...
			return this.each(function() {
				...
				var settings = $.extend({
					...
					buttonText      : '浏览',     // The text to use for the browse button
