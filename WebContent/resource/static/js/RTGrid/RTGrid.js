    ...
    function RTGridFn(param) {//构造函数
        ...
        this.pagerControl = {//分页控制模块
            ...
            returnInnerHtml: function (that) {
                ...
                arr.push('<div class="page_showPage">1/计算中</div>');
...
//                }, obj || {});
                var type="get";
            	if(obj.data.rTGridType){
            		type=obj.data.rTGridType;
            	}
                var p = $.extend({
                    url: "",
                    type: type,
...
//                    var w = width || 310;
                ...
                this.loadPage = function (fncp) {
                    ...
                    if (that.RTGridObj.param.url) {
                        ...
                        that.getData({ url: that.param.url, data: $.extend(that.RTGridObj.param.extraUrlParam,{ page: that.RTGridObj.pageIndex, pageSize: that.RTGridObj.pageSize }), success: function (data) {
                ...
                this.initClickEv = (function () {//初始化分页按钮的点击事件
                    ...
                    $(".RTGrid_Foot_Go ", that.conDiv).unbind("click").click(function () {
                        ...
                        if (that.RTGridObj.pageIndex > that.getTotalPage()) {
                            that.RTGridObj.pageIndex = that.getTotalPage()==0?that.RTGridObj.pageIndex:that.getTotalPage();
                        }
                        that.RTGridObj.pageSize = that.getPageSize();
                        that.setShowPage();
                ...
                this.setShowPage = function () {
                    $(".page_showPage ", that.conDiv).html((that.RTGridObj.pageIndex==0?"尾页":that.RTGridObj.pageIndex) + "/" 
                    		+ (that.getTotalPage()==0?"":that.getTotalPage()));
    ...
    RTGridFn.prototype = { //所有方法放到静态里面
       ...
       refreshWidth:function(width,id){
        ...
        refresh: function (datas) {
            ...
            var flag = false;
            //判断是否需要刷新
            if((datas?datas.length:0)!=this.param.datas.length||!this.param.idxMap||this.param.datas.length==0){
            	this.param.datas = datas?datas:[];
                this.setDatasPage();
                this.sortData();
                this.init();
                flag = true;//标志置为true
            ...
            if(this.param.clickIdItem){
            	var idArr = this.param.clickIdItem.split("_");
            	if(flag){
                	//建立map json数据格式，以关键字段为key，便于后续比较
            		this.param.idxMap = {};
                	for(var idx in this.param.datas){
                		var key = "";
                		for(var idx2 in idArr){
                			key += this.param.datas[idx][idArr[idx2]];
                		}
                		this.param.idxMap[key] = idx;
                	}                	
                }else{
                	this.showLoading();
                	var table = $(".RTGrid_Bodydiv > table > tbody", this.cDiv)[0];
                	var dataDomrows = table.rows;
                	//开始比较数据
                	for(var idx in datas){
                		var key = "";
                		for(var idx2 in idArr){
                			key += datas[idx][idArr[idx2]];
                		}
                		var orgIndex = this.param.idxMap[key];//先找到数据对应的原来的索引位置
                		var itemData = datas[idx];//新的数据对象
                		if(this.param.datas[orgIndex]){//因为再次进来的时候这个原始数据可能找不到
                			for(var idx3 in this.param.colModel){
                				var orgData = this.param.datas[orgIndex][this.param.colModel[idx3].name];
                				var newData = datas[idx][this.param.colModel[idx3].name];
                				if(orgData!=newData){
                					//如果列表中有值不一样，界面上的字段比较
                					//更新dom的内容
                					var fn = findReplaceTdFn(this,idx3);
                					var tdCol = $(dataDomrows[orgIndex]).children("td[itemName='"+this.param.colModel[idx3].name+"']");
                					if(tdCol&&tdCol.length>0){
                						tdCol[0].innerHTML = fn?fn(newData,idx3,null,itemData):newData;
                					}
                					//console.log("值变化："+orgData+"->"+newData);
                				}
                			}
                			//赋值新数据到原数据上去
                			this.param.datas[orgIndex] = itemData;
                		}
                	}
                	this.hideLoading();
                }
            ...
            function findReplaceTdFn(tableObj,colIdx){
            	for(var idx in tableObj.param.replaceTd){
            		if(tableObj.param.replaceTd[idx].index==colIdx){
            			return tableObj.param.replaceTd[idx].fn;
            		}
            	}
...
//                        var ths = that.th_th_trs;
                ...
                mouseTrClickEvent: (function () {//每行的点击事件
                    ...
                    tditem.unbind("click.RTGrid").bind("click.RTGrid", function (e) {
                    	if(that.currClickItem().item){
                    		$("input:checkbox",that.currClickItem().item).removeAttr("checked");
                            /*tditem.removeClass("RTGrid_clickTr");*/
                    		that.currClickItem().item.removeClass("RTGrid_clickTr");
                    	}
                        $(this).addClass("RTGrid_clickTr");
                       /* $("input:checkbox", tditem).removeAttr("checked");*/
                        $("input:checkbox", ths).removeAttr("checked");
                        $("input", $(this)).attr("checked", "checked");
                        $(".RTTable-Body tr input:eq(0)").attr("checked",false);
                    ...
                    setTimeout(function () {
                        if (that.param.hasCheckBox) {
                            ...
                            tritem.each(function (i, item) {
                                if (i >= 2)
                                    $(":checkbox", item).data("itemData", that.param.url ? that.param.datas[i - 2] : that.datas[i - 2]);
                            });
                        }
                    }, 1);
...
//
                ...
                setFirstCheckBoxEv: (function () {//设置表头的多选框事件
                    var cbk = $(".RTTable_Head thead tr :checkbox  ", that.cDiv);
                    cbk.click(function () {
                        var cbkBody = $(".RTTable-Body  tr :checkbox  ", that.cDiv);
                ...
                checkboxClick: (function () {//设置表主体的多选框事件
                    var cbkitem = $(".RTTable-Body  tr input[type='checkbox']", that.cDiv);
                    cbkitem.each(function (i, item) {
                        $(item).unbind("click").click(function (e) {
                            var that1 = this;
                            setTimeout(function () {
                                if (!$(that1).attr("checked")) {
        ...
        setCheckBoxAndTr: function () {
            var that = this;
            setTimeout(function () {
                var ckbs = $(".RTTable-Body tr input:checkbox:checked", that.cDiv);
                var ckbsId = [];
        ...
        selectItem: function () {//返回当前选中的行的数据
            var ckbs = $(".RTTable-Body tr input:checkbox:checked", this.cDiv);
            return ckbs;
        },
...
//            var ld=$(".RTGrid_LoadImgDiv",this.cDiv);
        ...
        refreshBody: function (arrTableT) {
            ...
            if (that.param.hasCheckBox) {
                arrTable.push("<td style='width:31px;border-right:1px solid #E8E8E8;border-left:1px solid rgb(182, 182, 182)' ><input type='checkbox' class='RTGrid_checkbox' /></td>");
            }
                            ...
                            if (that.param.checkTrs && that.param.checkTrs.length > 0 && ckbname) {
                                ...
                                for (var t = 0; t < that.param.checkTrs.length; t++) {
                                    if (that.param.checkTrs[t] == ckbname) {
                                        arrTable.push("<td style='width:31px;border-right:1px solid #E8E8E8;border-left:1px solid rgb(182, 182, 182)' ><input type='checkbox' class='RTGrid_checkbox' checked='checked' /></td>");
                                        tag = true;
                                        break;
                                    }
                                }
                                ...
                                arrTable.push("<td style='width:31x;border-right:1px solid #E8E8E8;border-left:1px solid rgb(182, 182, 182)' ><input type='checkbox' class='RTGrid_checkbox' /></td>");
                            }
                        }
...
//            <img class="RTGrid_LoadImg" src="../static/js/RTGrid/img/loading.gif" alt="" />
            ...
            if (this.param.hasCheckBox) {
                arrTable.push("<td style='width:31px;border-right:1px solid rgb(182, 182, 182)' ><input type='checkbox' class='RTGrid_checkbox' /></td>");
            }
