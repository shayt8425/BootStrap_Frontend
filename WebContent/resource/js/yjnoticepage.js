...
var typeNodes = [{value:'',name:'全部'},{value:'2',name:'库存到期'},{value:'3',name:'寿命到期'},{value:'4',name:'轨道车季检提醒'}];
...
function graph(){
	...
	FrontCommon.request("notice/pageList.json",params,function(data){
		...
		$.each(data.rows,function(key,value){
			...
			if(value.sender == "0"){
				value.sender_name = "dmp平台";
				value.sender_org_name = "系统消息"
			}
			...
			+"<th class='padding-left-10' width='200'><span class='sky'>通知类型："+type+"</span></th>"
			...
			+"<tr><td>"+value.sender_org_name+" "+value.sender_name+" 发来"+type+"</td></tr>";
			if(value.remark!=null){
				htm += "<tr><td>"+value.remark+"</td></tr>";
			}
			if(value.content!=null){
				 var conList = value.content.split(",");
				  htm += "<tr><td><div class=\"\" width='100%'>";
				  for (var j = 0; j < conList.length; j++) {
					  	if(value.type=='4'){
					  		htm += "<div class=\"box1\">" + conList[j] + "</div>";
					  	}else{
					  		htm += "<div class=\"box2\">" + conList[j] + "</div>";
					  	}
						
					}
				  htm += " <div style=\"clear:both;\"></div></div></td></tr>";
...
function fun(value){
	...
	var num = $("#yjnoticeNum", window.top.document).text();
	num = parseInt(num)-1;
	//alert(num)
	if(num==0){
